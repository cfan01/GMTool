...
******************/
app.findPaymemberNumberloss = function(query, cb){
...
//  });
    ...
    cb(data.PaymemberNumberloss_Info); //For Test ONLY! MUST remove if modify
}

app.findPaymemberNumberLossDetails = function(query, cb){
//  amqp.connect('amqp://'+Tools.CONFIG.rabbitmq, function(err, conn) {
//    conn.createChannel(function(err, ch) {
//      ch.assertQueue('', {exclusive: true}, function(err, q) {
//        var corr = generateUuid();
...
//  });
    //cb({"errCode":"Success"}); //For Test ONLY! MUST remove if modify
    cb(data.PaymemberNumberloss_Details); //For Test ONLY! MUST remove if modify
}
